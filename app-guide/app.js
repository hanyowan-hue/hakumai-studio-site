...
const AFFILIATE = {
  ...
  phone: { name: 'povo2.0', url: 'https://povo.jp/', priceNote: '基本料0円。ただし180日以上有料トッピング等がないと利用停止・解約の可能性あり' },
...
function buildRoute() {
  ...
  if (['google','both'].includes(state.platform)) {
    ...
    if (state.googleStatus === 'none') {
      if (state.googleDesired === 'individual') { addSection('googleNewIndividual'); steps.push('Google Play Consoleへ個人として登録する','新規アプリの本番公開前テスト要件を満たす'); cautions.push('2023年11月13日以降に作成された新しい個人アカウントでは、原則12人以上が14日間連続で参加するクローズドテスト後に本番アクセス申請が必要です。'); cautions.push('Google Playでは個人アカウントでも公開されるデベロッパー情報があり、有料販売やアプリ内課金を使う場合は住所表示にも注意が必要です。'); }
