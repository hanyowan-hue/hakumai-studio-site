...
function render() {
  ...
  else if (screen==='googlePublicWarning') html = page(`${info('⚠ 収益化すると、個人アカウントでも住所が出ることがあります',`有料アプリやアプリ内課金で収益化する場合、Google Paymentsプロファイルに登録した住所がGoogle Play上に表示される対象になります。<strong>自宅の住所で登録していれば、その自宅住所が公開される可能性があります</strong>。「個人アカウントなら住所は出ない」ではありません。 ${source(SOURCES.googlePublicInfo)}`,'warning')}${info('そのほかに公開される情報',`収益化しない場合でも、公開用のデベロッパー名・メールアドレスなどは表示されます。またGoogleは本人確認のため法的氏名・住所なども確認します。実際に表示される情報はアカウントの種類・収益化の方法・本人確認の状態などで変わるため、登録前に公式ヘルプで確認してください。`,'soft')}<div class="choices">${choice('確認した。問題ない','','googlePublicOk')}${choice('公開される情報が気になる','組織で登録する場合の条件も見てみる','googlePublicNg')}</div>`,'GOOGLE PLAY / 確認 2','収益化するなら、住所の公開も確認しておいてください');
