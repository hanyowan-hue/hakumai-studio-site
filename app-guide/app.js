...
const AFFILIATE = {
  virtualOffice: { name: 'METSバーチャルオフィス', url: 'https://vo-metsoffice.jp/', priceNote: '月270円〜(12か月一括のライトプラン。月払いは330円)' },
  phone: { name: 'povo2.0', url: 'https://povo.jp/', priceNote: '月額の基本料はかからないが、回線維持に180日ごとの有料トッピングが必要(最安クラスで年500円程度〜)' },
...
function buildRoute() {
  ...
  if (['google','both'].includes(state.platform)) {
    ...
    if (state.googleStatus === 'none') {
      if (state.googleDesired === 'individual') { addSection('googleNewIndividual'); steps.push('Google Play Consoleへ個人として登録する','新規アプリの本番公開前テスト要件を満たす'); cautions.push('2023年11月13日以降に作成された新しい個人アカウントでは、原則12人以上が14日間連続で参加するクローズドテスト後に本番アクセス申請が必要です。'); cautions.push('Google Playでは個人アカウントでも法的氏名・国・デベロッパーメールアドレスが公開されます。さらに有料販売やアプリ内課金で収益化する場合はPaymentsプロファイルの完全な住所も表示されるため、自宅住所で登録すると自宅が公開されます。'); }
...
function render() {
  ...
  else if (screen==='googleDecisionHelp') html = page(`${info('個人で登録すると',`2023年11月13日以降に作られた新しい個人アカウントでは、アプリを本番公開する前に、原則として<strong>12人以上のテスターが14日間連続で参加するテスト</strong>が必要です。アップデートのたびに毎回必要になるわけではありません。 ${source(SOURCES.googleTesting)}`,'warning')}${info('組織で登録すると',`D‑U‑N‑S番号(会社を識別する国際的な番号)や、組織名・住所・Webサイトなどの確認が増えます。 ${source(SOURCES.googleAccountType)}`,'soft')}<div class="choices">${choice('テスターを12人集められそう','個人で登録する','googleWantIndividual')}${choice('組織の準備をして進めたい','組織で登録する','googleWantOrg')}</div>`,'GOOGLE PLAY / 個人と組織の違い','個人は手軽に見えて、「公開前テスト」が一番のハードルです');
  ...
  else if (screen==='googlePublicWarning') html = page(`${info('⚠ 収益化すると、個人アカウントでも住所が公開されます',`有料アプリやアプリ内課金などで収益化する場合、Google Paymentsプロファイルに登録した<strong>完全な住所がGoogle Play上に表示されます</strong>。自宅の住所で登録していれば、自宅住所が公開されることになります。「個人アカウントなら住所は出ない」ではありません。 ${source(SOURCES.googlePublicInfo)}`,'warning')}${info('収益化しなくても公開される情報',`デベロッパー名は自分で設定でき、本名と同じにする必要はありません。ただしGoogle Play上には、<strong>法的氏名・国・デベロッパーメールアドレスが表示されます</strong>。表示項目は変更されることがあるため、登録前に公式ヘルプでも確認してください。`,'soft')}<div class="choices">${choice('確認した。問題ない','','googlePublicOk')}${choice('公開される情報が気になる','組織で登録する場合の条件も見てみる','googlePublicNg')}</div>`,'GOOGLE PLAY / 確認 2','収益化するなら、住所の公開も確認しておいてください');
  else if (screen==='googleOrgWarning') html = page(`${info('主な準備',`D‑U‑N‑S番号(会社を識別する国際的な番号)、組織名・住所、電話番号、Webサイト、Google Paymentsの組織情報などの確認が必要になります。 ${source(SOURCES.googleAccountType)}`,'soft')}${info('住所・電話番号などが公開されます',`組織アカウントでは、組織の名称・住所に加え、公開用として登録したデベロッパーのメールアドレス・電話番号などがGoogle Play上に表示されます。自宅の住所や個人の電話番号をそのまま出したくない場合は、住所レンタル(バーチャルオフィス)や事業用の電話番号を用意する方法があります。 ${source(SOURCES.googlePublicInfo)}`,'warning')}${info('自宅住所・個人の電話を出したくない場合の費用の目安',`<ul class="cost-list"><li><strong>住所レンタル</strong> … ${AFFILIATE.virtualOffice.priceNote}。例: <a class="source-link" href="${AFFILIATE.virtualOffice.url}" target="_blank" rel="noreferrer sponsored">${AFFILIATE.virtualOffice.name} ↗</a></li><li><strong>電話番号</strong> … ${AFFILIATE.phone.priceNote}。例: <a class="source-link" href="${AFFILIATE.phone.url}" target="_blank" rel="noreferrer sponsored">${AFFILIATE.phone.name} ↗</a></li></ul>プランによって郵便物の受け取りや法人登記の可否が違うので、契約前に各サービスの内容を確認してください。<span class="disclosure">${AFFILIATE.disclosure}</span>`,'soft')}<p class="lead">すでに個人アカウントを持っている場合は、条件を満たせば個人→組織への変更手続きがあります。 ${source(SOURCES.googleChangeType,'変更手順')}</p><div class="choices">${choice('わかった。組織ルートで進む','','googleOrgContinue')}</div>`,'GOOGLE PLAY / 組織登録','Google Playの組織登録は、準備するものが少し増えます');
  ...
  else if (screen==='googleExistingKeep') html = page(`${info('アカウントを作った時期を確認',`2023年11月13日以降に作った個人アカウントには、新しいアプリを本番公開する前のテスト条件(12人×14日間)があります。 ${source(SOURCES.googleTesting)}`,'warning')}${info('公開される情報',`個人アカウントでも、法的氏名・国・デベロッパーメールアドレスが公開されます。収益化する場合は完全な住所も表示されるため、現在の公式案内を確認してください。 ${source(SOURCES.googlePublicInfo)}`,'warning')}<div class="choices">${choice('確認した。個人のまま進む','','googleKeepContinue')}</div>`,'GOOGLE PLAY / 個人のまま使う','そのまま使う場合の確認ポイント');
  ...
  else if (screen==='address') html = page(`${info('⚠ なぜ聞くのか: 登録した住所は公開されることがあります',`Google Playの<strong>組織アカウントでは、法的住所がGoogle Play上に表示されます</strong>。<strong>個人アカウントでも、有料アプリやアプリ内課金で収益化する場合は、Paymentsプロファイルの完全な住所が表示されます</strong>。<br>つまり自宅の住所で登録すると、<strong>自宅住所が公開されます</strong>。それが気になる人だけ、住所を分けることを検討してください。 ${source(SOURCES.googlePublicInfo)}`,'warning')}<div class="choices">${choice('はい。自宅とは別の住所を使いたい','開業届やD‑U‑N‑Sの前に決めておくと、あとで住所変更する二度手間を減らせます','addressYes')}${choice('いいえ。自宅の住所で問題ない','自宅住所が公開される可能性は理解した上で進みます','addressNo')}${choice('まだ分からない','診断結果に「検討ポイント」として残します','addressMaybe')}</div>`,'最後の質問','事業用の住所を、自宅の住所と分けたいですか？','全員にバーチャルオフィスが必要なわけではありません。まず下の内容を確認してください。');
